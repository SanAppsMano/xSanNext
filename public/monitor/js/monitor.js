(() => {
  ...
  function handleCallAudio() {
    const n = state.dados && state.dados.ticketAtual.numero;
    if (!n || n === lastTicket) return;
    if (alertSound) {
      alertSound.currentTime = 0;
      alertSound.play().catch(() => {});
    }
    if ('speechSynthesis' in window) {
      const tipo = state.dados.ticketAtual.tipo === 'Preferencial' ? 'preferencial' : '';
      const guiche = state.dados.ticketAtual.guiche ? ` guichê ${state.dados.ticketAtual.guiche}` : '';
      const utter = new SpeechSynthesisUtterance(`Senha ${n} ${tipo}${guiche}`.trim());
      utter.lang = 'pt-BR';
